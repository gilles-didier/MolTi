...
static void *simulThread(void *data) {
	double dist, x, cmp, cmpS, cmpI, cmpU, cmpC;
	...
	part2 = getPartitionConsensus(graph, LouvainType, &alp);
	pthread_mutex_lock (&set.mutex_arg);
...
//	srand(time(NULL));
	...
	if(fo = fopen(outputFileName, "w")) {
		double std, stdS, stdI, stdU, stdC;
...
fprintf (stderr, "i %d threads %d\n", i, set.number);
	...
	pthread_mutex_lock (&set.mutex_arg);
			...
			fprintf(fo, "%d\t%lE\t%lE\t%lE\t%lE\t%lE\t%lE\t%lE\t%lE\t%lE\t%lE", nTable, set.arg.mean, std, set.arg.meanS, stdS, set.arg.meanI, stdI, set.arg.meanU, stdU, set.arg.meanC, stdC);
			fprintf(fo, "\n");
			fprintf(stdout, "%d\t%lE\t%lE\t%lE\t%lE\t%lE\n", nTable, set.arg.mean, set.arg.meanS, set.arg.meanI, set.arg.meanU, set.arg.meanC);
	pthread_mutex_unlock (&set.mutex_arg);
		}
