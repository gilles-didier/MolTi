...
QItemIntDelegate::QItemIntDelegate() : QStyledItemDelegate()
{

}

QItemIntDelegate::~QItemIntDelegate()
{

}

QString QItemIntDelegate::displayText(const QVariant & value, const QLocale & locale) const
{
    bool ok;
    int v = value.toInt(&ok);
    if(ok)
        return locale.toString(v);
    else
        return QString("-");
}
